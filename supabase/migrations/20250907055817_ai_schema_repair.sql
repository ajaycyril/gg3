-- Repair migration: ensure AI schema and monetization tables exist
create extension if not exists pgcrypto;
create extension if not exists vector;
create extension if not exists pg_trgm;

-- Variants & components (idempotent)
...
-- Expression-based unique identity
create unique index if not exists uq_components_identity on components (type, coalesce(vendor,''), coalesce(model,''));
...
create unique index if not exists uq_component_benchmarks_identity on component_benchmarks (component_id, suite, metric, coalesce(run_date, 'epoch'::date));
...
-- Views for search cards and scores
...
-- RLS toggles
alter table if exists review_chunks enable row level security;
alter table if exists aspect_opinions enable row level security;
alter table if exists content_embeddings enable row level security;

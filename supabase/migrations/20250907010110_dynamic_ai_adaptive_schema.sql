...
-- Ensure expires_at exists when table pre-exists from earlier migrations
ALTER TABLE conversation_sessions
  ADD COLUMN IF NOT EXISTS expires_at TIMESTAMPTZ DEFAULT (NOW() + INTERVAL '24 hours');

-- Ensure embedding columns exist for indexes when tables pre-exist
ALTER TABLE user_profiles
  ADD COLUMN IF NOT EXISTS preference_embedding VECTOR(1024);
ALTER TABLE conversation_sessions
  ADD COLUMN IF NOT EXISTS session_embedding VECTOR(768);
...
-- Enhanced gadgets table for dynamic AI processing (compatibility)
ALTER TABLE gadgets ADD COLUMN IF NOT EXISTS ai_generated_description TEXT;
ALTER TABLE gadgets ADD COLUMN IF NOT EXISTS target_personas JSONB DEFAULT '{}';
ALTER TABLE gadgets ADD COLUMN IF NOT EXISTS complexity_suitability JSONB DEFAULT '{}';
ALTER TABLE gadgets ADD COLUMN IF NOT EXISTS dynamic_tags JSONB DEFAULT '[]';
ALTER TABLE gadgets ADD COLUMN IF NOT EXISTS ai_insights JSONB DEFAULT '{}';
ALTER TABLE gadgets ADD COLUMN IF NOT EXISTS comparison_highlights JSONB DEFAULT '{}';
...
CREATE INDEX IF NOT EXISTS idx_conversation_sessions_active 
  ON conversation_sessions(user_id, created_at DESC, expires_at);
...
-- User can only access their own data (idempotent, type-safe)
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='user_profiles' AND policyname='Users own their profiles'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='conversation_sessions' AND policyname='Users own their conversations'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='adaptive_ui_configs' AND policyname='Users own their UI configs'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='ai_conversation_logs' AND policyname='Users own their conversation logs'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='dynamic_recommendations' AND policyname='Users see their own recommendations'
  ) THEN

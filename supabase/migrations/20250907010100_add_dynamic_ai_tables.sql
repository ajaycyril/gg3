...
-- RLS Policies for anonymous and authenticated users (idempotent)
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'user_profiles' AND policyname = 'Users can manage their own profiles'
  ) THEN
    CREATE POLICY "Users can manage their own profiles" ON public.user_profiles
      FOR ALL USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'conversation_sessions' AND policyname = 'Users can manage their own sessions'
  ) THEN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'conversation_turns' AND policyname = 'Users can manage their own conversation turns'
  ) THEN
    CREATE POLICY "Users can manage their own conversation turns" ON public.conversation_turns
      FOR ALL USING (user_id = COALESCE(auth.uid()::text, 'anonymous'));
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'ui_configurations' AND policyname = 'Users can manage their own UI configurations'
  ) THEN
    CREATE POLICY "Users can manage their own UI configurations" ON public.ui_configurations
      FOR ALL USING (user_id = COALESCE(auth.uid()::text, 'anonymous'));
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'user_interactions' AND policyname = 'Users can manage their own interactions'
  ) THEN
